...
const RegistryConfigSchema = z.object({
  name: z.string().regex(/^[a-z0-9-]+$/, 'Registry name must be lowercase alphanumeric with hyphens'),
...
`
    ...
    console.log(`   2. Update registries/${validated.name}.json with component entries`)
    console.log(`   3. Run: bun run registry:build`)
    console.log(`   4. Test: curl http://localhost:3000/r/${validated.name}/index.json`)
...
async function createBasicExampleComponent(registryName: string, registryDir: string) {
  ...
  await fs.writeFile(path.join(componentDir, 'welcome-card.tsx'), componentContent)
...
`)
  ...
  if (!flags.display || !flags.description || !flags.author) {
    console.error('❌ Missing required options: --display, --description, --author')
    process.exit(1)
  }
