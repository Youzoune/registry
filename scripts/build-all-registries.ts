...
async function buildAllRegistries() {
  try {
    ...
    if (await exists(registriesDir)) {
      ...
      console.log(
        '📦 Using legacy registry.json (consider migrating to registries/ directory)',
      )
    } else {
      console.error('❌ No registry configuration found')
      console.log(
        '💡 Run: bun run scripts/create-registry.ts my-registry --display="My Registry" --description="..." --author="..."',
      )
      process.exit(1)
    }
    ...
    for (const registryFile of registryFiles) {
      ...
      try {
        ...
        const registryName =
          registry.name || path.basename(registryFile, '.json')
        const isLegacy = registryFile.endsWith('registry.json')
        const outputDir = isLegacy
          ? publicRDir
          : path.join(publicRDir, registryName)
        ...
        console.log(
          `✅ Registry "${registryName}" built successfully (${registry.registry.length} components)`,
        )
      } catch (error) {
    ...
    if (
      registryFiles.length > 1 ||
      !registryFiles[0].endsWith('registry.json')
    ) {
      const globalIndex = {
        name: 'registries',
